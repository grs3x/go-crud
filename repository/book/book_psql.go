...
func (b BookRepository) UpdateBook(db *sql.DB, book models.Book) (int64, error) {
	...
	results, err := db.Exec("UPDATE books SET title=$1, author=$2, year=$3 WHERE ID id=$4 RETURNING id", &book.Title, &book.Author, &book.Year, &book.ID)
...
func (b BookRepository) RemoveBook(db *sql.DB, id int) (int64, error) {
	...
	results, err := db.Exec("DELETE FROM books WHERE id=$1", id)
